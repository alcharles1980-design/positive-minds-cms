...
### CONNECTING: what "Connection issue" actually means, and how to diagnose it

A connector that authenticates and then shows as disconnected is NOT a server fault, and this cost
most of a session to establish. Written down so nobody re-runs it.

**WHAT THE EVIDENCE LOOKED LIKE.** Six connections in fifteen minutes, every one registering a BRAND
NEW client, every access token used within seconds of being issued — and the connectors page still
showing "Connection has expired". Server logs full of 200s.

**HOW IT WAS SETTLED: drive the real page in a real browser.** Playwright + Chromium, mobile profile,
recording every request and navigation:
    GET  /mcp/authorize                          200   page renders
    POST /mcp/authorize                          200   token accepted
    GET  claude.ai/api/mcp/auth_callback?code=   307
    GET  claude.ai/settings/connectors?step=end_error&flow_id=ofid_...
Our side is clean end to end. The code is issued, the browser reaches the callback, and CLAUDE'S OWN
FLOW ends in \`end_error\`.

**WHAT flow_id MEANS.** It is Claude's state, created when Connect is pressed and consumed at the
callback. If the flow open when the callback arrives is not the one that started it, it ends in
end_error. So a second Connect press, or the phone handing the redirect to the Claude app, kills the
flow the callback belongs to. That is why each attempt registered a fresh client and why RETRYING
MAKES IT WORSE — the instinct that fixes most things makes this one unrecoverable.

**THE SEQUENCE THAT WORKS:** delete every entry for the connector, close every Claude tab AND the
app, then one tab, add once, press Connect ONCE, and do not press it again while it is in flight.

**WHAT IS NOT WORTH DOING.** Two remedies were tried and both were wrong. Telling the partner to
paste the callback URL into the address bar starts ANOTHER transient add rather than completing the
original — it walks them round the loop. And detecting the hijack from inside the page cannot work:
when the app takes the foreground the tab is backgrounded, so \`document.hidden\` goes true and any
guard against false positives suppresses the true one. Both were shipped into the sign-in page and
both were removed. See rule 4.39.
